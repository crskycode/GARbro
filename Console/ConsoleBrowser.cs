...
namespace GARbro {
	...
	class ConsoleBrowser {
		...
		Stream CreateNewFile(string filename) {
			...
			if (File.Exists(path) && !alwaysOverwrite) {
		...
		void ProcessArchiveFile(ConsoleCommand command) {
			...
			//Console.WriteLine("File offset : {0}", fileArray[0].Offset);
		...
		static void Usage() {
			...
			Console.WriteLine("  -y               Do not prompt if the same file exist. Will always overwrite.");
